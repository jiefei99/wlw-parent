...
import com.jike.wlw.service.equipment.BatchImportRq;
import com.jike.wlw.service.equipment.BatchRegisterDeviceRq;
import com.jike.wlw.service.equipment.BatchUpdateDeviceNicknameRq;
...
import com.jike.wlw.service.equipment.EquipmentQueryNameByProductRq;
import com.jike.wlw.service.equipment.EquipmentSqlFilter;
import com.jike.wlw.service.equipment.EquipmentStatisticsQueryRq;
import com.jike.wlw.service.equipment.QueryDeviceGroupListFilter;
...
public class SysWebEquipmentController extends BaseController {
        ...
        try {
            ...
            if (SourceTypes.ALIYUN.equals(source.getData().getType())) {
                if (StringUtils.isNotBlank(filter.getName())) {
                    ...
                    for (Equipment equipment : result.getData()) {
                        EquipmentByNameVO equipmentByNameVO = new EquipmentByNameVO();
                        BeanUtils.copyProperties(equipment, equipmentByNameVO);
                        list.add(equipmentByNameVO);
                    }
    ...
    ActionResult<ImportData> batchImport(@ApiParam(required = true, value = "产品Key") @RequestBody BatchImportRq importRq) throws BusinessException {
        try {
            ...
            if (SourceTypes.ALIYUN.equals(source.getData().getType())) {
                result = aliEquipmentFeignClient.batchImport(getTenantId(), importRq.getProductKey(), importRq.getFilePath());
            } else {
                return ActionResult.fail("暂时只支持阿里云资源");
            }
    ...
    ActionResult<List<String>> batchSqlQueryByVersion(@ApiParam(required = true, value = "") @RequestBody EquipmentSqlFilter filter) throws BusinessException {
        try {
            ...
            if (SourceTypes.ALIYUN.equals(source.getData().getType())) {
                result = aliEquipmentFeignClient.queryDeviceVersionBySQL(getTenantId(), filter);
            } else {
                return ActionResult.fail("暂时只支持阿里云资源");
            }
